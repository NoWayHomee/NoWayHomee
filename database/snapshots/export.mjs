  ...
  return sql
    ...
    .filter((line) => {
      ...
      return (
        ...
        !trimmed.startsWith("-- Generated by database/snapshots/export.mjs")
      );
    })
    .join("\n");
}
...
export async function exportSchemaSync() {
  ...
  let existingContent = "";
  try {
    existingContent = await fs.readFile(schemaOut, "utf8");
  } catch (error) {
    // File doesn't exist
  }

  if (cleanSql(existingContent) !== cleanSql(content)) {
    await fs.writeFile(schemaOut, content, "utf8");
    console.log(`Updated schema snapshot: ${schemaOut}`);
  } else {
    console.log(`No schema changes, skipping write.`);
  }
}
...
export async function exportDataSync() {
  ...
  let existingContent = "";
  try {
    existingContent = await fs.readFile(dataOut, "utf8");
  } catch (error) {
    // File doesn't exist
  }
  ...
  if (cleanSql(existingContent) !== cleanSql(content)) {
    ...
    console.log(`Updated data snapshot: ${dataOut}`);
  } else {
    console.log(`No data changes, skipping write.`);
  }
}
